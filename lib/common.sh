...
# Get timing template based on mode and site
aggressive_timing() {
    local site="${1:-}"
...
# Get rate limit based on mode (returns multiplied rate for aggressive)
aggressive_rate() {
    local base_rate=$1
    if is_aggressive; then
        echo $(( base_rate * 3 ))
...
# Confirmation prompt for aggressive mode — must be called before scanning
require_aggressive_confirmation() {
    echo ""
    echo -e "${RED}╔════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${RED}║  ⚠  WARNING: AGGRESSIVE SCAN MODE                        ║${NC}"
    echo -e "${RED}╠════════════════════════════════════════════════════════════╣${NC}"
    echo -e "${RED}║  This mode runs INTRUSIVE scans including:                ║${NC}"
    echo -e "${RED}║    • Exploit validation scripts                           ║${NC}"
    echo -e "${RED}║    • Credential brute-force attempts                      ║${NC}"
    echo -e "${RED}║    • Full port scans at high rates                        ║${NC}"
    echo -e "${RED}║    • Active protocol probing                              ║${NC}"
    echo -e "${RED}║                                                           ║${NC}"
    echo -e "${RED}║  This WILL generate significant network traffic and       ║${NC}"
    echo -e "${RED}║  may trigger IDS/IPS alerts. Ensure you have explicit     ║${NC}"
    echo -e "${RED}║  written authorization before proceeding.                 ║${NC}"
    echo -e "${RED}╚════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    read -rp "$(echo -e "${RED}Type 'CONFIRM' to proceed with aggressive scanning: ${NC}")" answer
    if [[ "$answer" != "CONFIRM" ]]; then
        error "Aggressive mode not confirmed. Aborting."
        exit 1
    fi
    echo ""
    warn "Aggressive mode confirmed. Proceeding with intrusive scans."
    echo ""
}
